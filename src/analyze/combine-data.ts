...
import {CardLocation, CardRecord} from './collect-cards';
...
export async function compileCards(entry: EntryInfo): Promise<void> {
  ...
  const cardIndex: { [name: string]: CardRecord } = JSON.parse(await fs.promises.readFile(`${entry.out}/json/cardIndex.json`, {encoding: 'utf8'}));
  ...
  await Promise.allSettled(allNames.map(name => compileItemCard(name, entry, categoryData[name], listIndex[name], cardIndex[name])));
}
...
async function loadCardSources(entry: EntryInfo, cardRecord: CardRecord): Promise<ScrappedItemWithSource[]> {
  ...
  result.forEach((item, i) => {
    if (cardRecord.commonName)
      item.commonName = cardRecord.commonName;
